...
static inline void wait_spin(void)
{
#if defined(_MSC_VER)
#  if defined(_M_ARM64)
  __yield();
...
//! Size classes
#define SCLASS(n) {(n * SMALL_GRANULARITY), (SMALL_PAGE_SIZE - PAGE_HEADER_SIZE) / (n * SMALL_GRANULARITY)}
#define MCLASS(n) {(n * SMALL_GRANULARITY), (MEDIUM_PAGE_SIZE - PAGE_HEADER_SIZE) / (n * SMALL_GRANULARITY)}
#define LCLASS(n) {(n * SMALL_GRANULARITY), (LARGE_PAGE_SIZE - PAGE_HEADER_SIZE) / (n * SMALL_GRANULARITY)}
...
static heap_t *heap_allocate(int first_class);

static void heap_page_free_decommit(heap_t *heap, uint32_t page_type, uint32_t page_retain_count);
...
static heap_t *get_thread_heap_allocate(void)
{
...
static inline uint32_t get_size_class_tiny(size_t size)
{
...
static inline page_type_t get_page_type(uint32_t size_class)
{
...
static inline size_t get_page_aligned_size(size_t size)
{
...
static void os_set_page_name(void *address, size_t size)
{
#if defined(__linux__) || defined(__ANDROID__)
...
static void *os_mmap(size_t size, size_t alignment, size_t *offset, size_t *mapped_size)
{
...
static int os_mcommit(void *address, size_t size)
{
#if ENABLE_DECOMMIT
  if (global_config.disable_decommit)
  {
...
static int os_mdecommit(void *address, size_t size)
{
#if ENABLE_DECOMMIT
  if (global_config.disable_decommit)
    return 1;
...
static void os_munmap(void *address, size_t offset, size_t mapped_size)
{
...
static inline int page_commit_memory_pages(page_t *page)
{
  if (!page->is_decommitted)
    return 0;
  void *extra_page = pointer_offset(page, global_config.page_size);
...
static void page_available_to_full(page_t *page)
{
  heap_t *heap = page->heap;
  if (heap->page_available[page->size_class] == page)
  {
...
static inline void page_put_local_free_block(page_t *page, block_t *block)
{
...
static NOINLINE void page_adopt_thread_free_block_list(page_t *page)
{
  if (page->local_free)
    return;
...
static NOINLINE void page_put_thread_free_block(page_t *page, block_t *block)
{
...
static void page_push_local_free_to_heap(page_t *page)
{
...
static NOINLINE void *page_initialize_blocks(page_t *page)
{
...
static inline RPMALLOC_ALLOCATOR void *page_allocate_block(page_t *page, unsigned int zero)
{
...
static inline int span_is_thread_heap(span_t *span)
{
#if RPMALLOC_FIRST_CLASS_HEAPS
...
static inline page_t *span_get_page_from_block(span_t *span, void *block)
{
  return (page_t *) ((uintptr_t) block & span->page_address_mask);
}

//! Find or allocate a page from the given span
static inline page_t *span_allocate_page(span_t *span)
{
...
static NOINLINE void span_deallocate_block(span_t *span, page_t *page, void *block)
{
...
static inline span_t *block_get_span(block_t *block)
{
  return (span_t *) ((uintptr_t) block & SPAN_MASK);
}

static inline void block_deallocate(block_t *block)
{
...
static inline size_t block_usable_size(block_t *block)
{
  span_t *span = (span_t *) ((uintptr_t) block & SPAN_MASK);
...
static inline heap_t *heap_initialize(void *block)
{
  heap_t *heap = block;
...
static heap_t *heap_allocate_new(void)
{
...
static void heap_unmap(heap_t *heap)
{
...
static inline void heap_release(heap_t *heap)
{
  heap_lock_acquire();
  if (heap->prev)
...
static inline span_t *heap_get_span(heap_t *heap, page_type_t page_type)
{
...
static page_t *heap_get_page(heap_t *heap, uint32_t size_class);

static void block_deallocate(block_t *block);

static page_t *heap_get_page_generic(heap_t *heap, uint32_t size_class)
{
...
static inline RPMALLOC_ALLOCATOR void *heap_pop_local_free(heap_t *heap, uint32_t size_class)
{
...
static NOINLINE RPMALLOC_ALLOCATOR void *heap_allocate_block_small_to_large(heap_t *heap, uint32_t size_class,
                                                                            unsigned int zero)
{
...
//! Generic allocation path from heap pages, spans or new mapping
static NOINLINE RPMALLOC_ALLOCATOR void *heap_allocate_block_huge(heap_t *heap, size_t size, unsigned int zero)
{
  if (heap->id == 0)
  {
    // Thread has not yet initialized, assign heap and try again
...
static RPMALLOC_ALLOCATOR NOINLINE void *heap_allocate_block_generic(heap_t *heap, size_t size, unsigned int zero)
{
...
static inline RPMALLOC_ALLOCATOR void *heap_allocate_block(heap_t *heap, size_t size, unsigned int zero)
{
  if (size <= (SMALL_GRANULARITY * 64))
  {
...
static RPMALLOC_ALLOCATOR void *heap_allocate_block_aligned(heap_t *heap, size_t alignment, size_t size,
                                                            unsigned int zero)
{
...
static void *heap_reallocate_block(heap_t *heap, void *block, size_t size, size_t old_size, unsigned int flags)
{
  if (block)
  {
...
static void *heap_reallocate_block_aligned(heap_t *heap, void *block, size_t alignment, size_t size, size_t old_size,
                                           unsigned int flags)
{
  if (alignment <= SMALL_GRANULARITY)
...
static void heap_free_all(heap_t *heap)
{
...
extern RPMALLOC_ALLOCATOR void *rpaligned_realloc(void *ptr, size_t alignment, size_t size, size_t oldsize,
                                                  unsigned int flags)
{
#if ENABLE_VALIDATE_ARGS
...
extern void rpmalloc_thread_finalize(void)
{
  heap_t *heap = get_thread_heap();
...
extern void rpmalloc_thread_collect(void)
{
}
...
void rpmalloc_global_statistics(rpmalloc_global_statistics_t *stats)
{
  ...
  stats->mapped_peak =
      global_config.page_size * atomic_load_explicit(&global_statistics.page_mapped_peak, memory_order_relaxed);
  stats->committed =
      global_config.page_size * atomic_load_explicit(&global_statistics.page_commit, memory_order_relaxed);
  stats->decommitted =
      global_config.page_size * atomic_load_explicit(&global_statistics.page_decommit, memory_order_relaxed);
  stats->active = global_config.page_size * atomic_load_explicit(&global_statistics.page_active, memory_order_relaxed);
  stats->active_peak =
      global_config.page_size * atomic_load_explicit(&global_statistics.page_active_peak, memory_order_relaxed);
...
RPMALLOC_ALLOCATOR void *rpmalloc_heap_aligned_zalloc(rpmalloc_heap_t *heap, size_t alignment, size_t size)
{
#  if ENABLE_VALIDATE_ARGS
  if (size >= MAX_ALLOC_SIZE)
  {
...
extern inline RPMALLOC_ALLOCATOR void *rpmalloc_heap_aligned_calloc(rpmalloc_heap_t *heap, size_t alignment, size_t num,
                                                                    size_t size)
{
  size_t total;
#  if ENABLE_VALIDATE_ARGS
...
RPMALLOC_ALLOCATOR void *rpmalloc_heap_aligned_realloc(rpmalloc_heap_t *heap, void *ptr, size_t alignment, size_t size,
                                                       unsigned int flags)
{
#  if ENABLE_VALIDATE_ARGS
  if ((size + alignment < size) || (alignment > SMALL_PAGE_SIZE))
...
struct rpmalloc_heap_statistics_t rpmalloc_heap_statistics(rpmalloc_heap_t *heap)
{
#  if RPMALLOC_HEAP_STATISTICS
  if (heap)
  {
